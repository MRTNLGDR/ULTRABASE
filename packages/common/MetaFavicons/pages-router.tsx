...
const MetaFaviconsPagesRouter = ({
  ...
  route?: string
  // optional product-specific icon in the selected favicon directory
  iconFile?: string
  ...
  return (
    <Head>
      ...
      <link rel="shortcut icon" href={`${basePath}${route}/${iconFile}`} />
      <link
        rel="icon"
        type={iconFile.endsWith('.svg') ? 'image/svg+xml' : 'image/x-icon'}
        href={`${basePath}${route}/${iconFile}`}
      />
      <link rel="apple-touch-icon" href={`${basePath}${route}/${iconFile}`} />
      {includeRssXmlFeed && (
