...
# Ultrabase is bound exclusively to 127.0.0.1 by its local Compose overlay.
# In this mode, remove the browser Basic Auth prompt so the owner's local
# profile opens the Studio directly. Credentials remain generated and stored
# for recovery or for re-enabling authentication before network publication.
if [ "$ULTRABASE_LOCAL_AUTO_ACCESS" = "true" ]; then
  sed -i '/^      - name: basic-auth$/,+2d' "$KONG_DECLARATIVE_CONFIG"
fi
